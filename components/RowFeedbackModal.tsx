...
// Definiera radtypen separat — ingen referens till Props
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 201, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, pointerEvents: "none" }}>
        <div style={{ pointerEvents: "all", background: "var(--bg-elevated)", border: "0.5px solid var(--border-strong)", borderRadius: "var(--radius-lg)", width: "100%", maxWidth: 440, padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 15, fontWeight: 600, color: "var(--text-primary)" }}>Justera rad</div>
          ...
          <div style={{ background: "var(--bg-surface)", border: "0.5px solid var(--border)", borderRadius: "var(--radius)", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <input className="input" type="number" value={quantity} onChange={e => setQuantity(e.target.value)} style={{ width: 90, textAlign: "right", borderColor: qtyChanged ? "var(--accent)" : undefined, fontWeight: qtyChanged ? 600 : 400 }} />
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <input className="input" type="number" value={unitPrice} onChange={e => setUnitPrice(e.target.value)} style={{ width: 90, textAlign: "right", borderColor: priceChanged ? "var(--accent)" : undefined, fontWeight: priceChanged ? 600 : 400 }} />
            ...
            <div style={{ borderTop: "0.5px solid var(--border)", paddingTop: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {anyChanged && <span style={{ fontSize: 12, color: "var(--text-faint)", textDecoration: "line-through" }}>{fmtKr(row.total)}</span>}
                <span style={{ fontSize: 14, fontWeight: 600, color: anyChanged ? "var(--text-primary)" : "var(--text-secondary)" }}>{fmtKr(newTotal)}</span>
              </div>
            </div>
          </div>
          ...
          {anyChanged && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: "0.5px" }}>Varför ändrar du?</label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {REASON_OPTIONS.map(r => (
                  <button key={r.code} onClick={() => setReasonCode(r.code)} style={{ padding: "4px 10px", borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: "pointer", border: "0.5px solid", borderColor: reasonCode === r.code ? "var(--accent)" : "var(--border-strong)", background: reasonCode === r.code ? "var(--accent)" : "transparent", color: reasonCode === r.code ? "#fff" : "var(--text-secondary)", transition: "all 0.1s" }}>
                    {r.label}
                  </button>
                ))}
              </div>
              <textarea className="input textarea" placeholder={needsText ? "Beskriv kortfattat varför… (obligatoriskt)" : "Valfri kommentar…"} value={reasonText} onChange={e => setReasonText(e.target.value)} rows={2} style={{ resize: "none", borderColor: needsText && !reasonText.trim() && error ? "var(--red)" : undefined }} />
            </div>
          )}

          {error && (
            <div style={{ fontSize: 12, color: "var(--red)", padding: "8px 12px", background: "rgba(220,50,50,0.08)", borderRadius: "var(--radius)" }}>{error}</div>
          )}
