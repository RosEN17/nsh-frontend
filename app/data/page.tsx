...
function FortnoxSection({ onSyncDone }: { onSyncDone: () => void }) {
  ...
  useEffect(() => {
    if (!companyId || !API_BASE) { setStatus("disconnected"); return; }
  ...
  async function connectFortnox() {
    if (!companyId) {
      setErrorMsg("Inget bolag kopplat till ditt konto.");
      return;
    }
    try {
      ...
      else setErrorMsg(data.detail || "Kunde inte skapa auth-URL.");
    } catch (err: any) {
  ...
  async function disconnectFortnox() {
    ...
    try {
      await fetch(`${API_BASE}/api/fortnox/disconnect?company_id=${companyId}`, { method: "POST" });
      setStatus("disconnected");
      setCompanyInfo(null);
      setScope("");
      setSyncMsg("");
    ...
    <div className="data-section-card">
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-faint)", marginTop: 1 }}>
            {status === "connected"
              ...
              : status === "syncing" ? "Hämtar data..."
              : status === "connecting" ? "Kopplar..."
              : "Inte kopplad"}
          </div>
        </div>
        {status === "connected" && (
      ...
      {(status === "disconnected" || status === "error") && (
        <div>
          ...
          <button onClick={connectFortnox} style={{
      ...
      {status === "connecting" && (
        ...
        }}><Spinner /> Kopplar Fortnox — vänta...</div>
      )}
          ...
          {companyInfo && (
            <div style={{
              ...
              {companyInfo.OrganizationNumber && (
                <div><span style={{ color: "var(--text-faint)" }}>Org.nr: </span><span style={{ color: "var(--text-muted)" }}>{companyInfo.OrganizationNumber}</span></div>
              )}
              {(companyInfo.CompanyName || companyInfo.Name) && (
                <div><span style={{ color: "var(--text-faint)" }}>Bolag: </span><span style={{ color: "var(--text-muted)" }}>{companyInfo.CompanyName || companyInfo.Name}</span></div>
              )}
              {companyInfo.City && (
                <div><span style={{ color: "var(--text-faint)" }}>Ort: </span><span style={{ color: "var(--text-muted)" }}>{companyInfo.City}</span></div>
              )}
              {scope && (
                <div><span style={{ color: "var(--text-faint)" }}>Scope: </span><span style={{ color: "var(--text-muted)" }}>{scope}</span></div>
              )}
            </div>
          )}
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={syncFortnox} className="ns-btn-primary" style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                <path d="M2 8a6 6 0 0111.47-2.47M14 8a6 6 0 01-11.47 2.47" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
            ...
            <button onClick={disconnectFortnox} style={{
      ...
      {status === "syncing" && (
        <div style={{
          ...
          background: "var(--accent-soft)", border: "0.5px solid var(--accent-border)",
          color: "var(--accent-text)", fontSize: 12,
        }}><Spinner /> Hämtar kontoplan och verifikationer från Fortnox...</div>
      )}
    </div>
  );
}
...
export default function DataPage() {
  ...
  useEffect(() => { setSavedMeta(getFileMeta()); }, []);

  function handleFile(f: File | null) {
    setFile(f); setError(null); setAnalysis(null); setMapping({}); setAiState("idle");
  }

  function handleRemoveFile() {
    clearAll(); setSavedMeta(null); setFile(null); setAnalysis(null); setMapping({}); setAiState("idle");
  }

  async function runAiMapping() {
    if (!file) return;
    setError(null); setAiState("running");
    try {
      ...
      for (const field of MAPPING_FIELDS) {
        ...
        if (suggestions.length > 0) { auto[field.key] = suggestions[0]; continue; }
        const best = smartMatch(field.key, field.label, cols, preview);
        if (best) auto[field.key] = best;
      }
      setMapping(auto); setAiState("done");
    } catch (e: any) {
      setError(e.message || "Något gick fel vid filanalysen."); setAiState("idle");
    }
  }
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      saveFileMeta(meta); setSavedMeta(meta);
    ...
    <ProtectedLayout>
      ...
      <div className="ns-page">
        <div className="ns-hero">
          <div className="ns-hero-title">Data</div>
          <div className="ns-hero-sub">
            Koppla Fortnox direkt eller ladda upp Excel / CSV — allt på ett ställe.
          </div>
        </div>
        ...
        <div className="data-section-card">
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              flexShrink: 0, color: "var(--accent-text)", fontSize: 18,
            }}>⬆</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)" }}>Filuppladdning</div>
              <div style={{ fontSize: 11, color: "var(--text-faint)", marginTop: 1 }}>CSV, XLSX eller XLS</div>
            </div>
          </div>
          ...
          {savedMeta && !file && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                <button className="ns-btn-primary" onClick={() => router.push("/dashboard")}
                  ...
                  <div>
                    ...
                    <div className="ns-dropzone-filesize">
                      ...
                      <span style={{ cursor: "pointer", textDecoration: "underline" }}
                        onClick={(e) => { e.stopPropagation(); handleFile(null); }}>byt fil</span>
                    </div>
                  </div>
                </div>
              ) : (
          ...
          {file && aiState === "idle" && (
            <button className="ns-btn-primary" onClick={runAiMapping}
              style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 12 }}>
          ...
          {file && aiState === "running" && (
            <div style={{
              ...
              background: "var(--accent-soft)", border: "0.5px solid var(--accent-border)",
              color: "var(--accent-text)", fontSize: 12,
            }}><Spinner /> AI analyserar kolumner och format...</div>
          )}
        </div>

        {/* ── Mapping card (outside the section card, below) ── */}
          ...
          <>
            ...
            <div className="ns-mapping-card">
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8,
                  ...
                  <span style={{
                    ...
                    background: "var(--green-soft)", color: "var(--green)",
                  }}>{totalMapped}/{MAPPING_FIELDS.length} mappade</span>
                </div>
                  ...
                  return (
                    <div key={field.key} className="ns-mapping-row">
                      <div style={{ display: "flex", alignItems: "center",
                        ...
                        <label className="ns-mapping-label" style={{ cursor: "help" }}
                      ...
                      <select className="ns-mapping-select"
              ...
              <div style={{
                ...
                <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                  ...
                  <button className="ns-btn-primary" onClick={runAnalysis}
            ...
            {analysis.preview?.length > 0 && (
              <div style={{
                ...
                }}>Förhandsgranskning — {Math.min(analysis.preview.length, 5)} rader</div>
                        ...
                        {previewCols.map((col) => {
                          ...
                          return (
                            <th key={col} style={{
                              ...
                              {col}{isMapped && <span style={{ opacity: 0.7, marginLeft: 4 }}>→ {fieldName}</span>}
                            </th>
                          );
                        })}
                      </tr>
...
function smartMatch(
  fieldKey: string, fieldLabel: string, cols: string[], preview: Record<string, any>[]
): string | null {
  const SYNONYMS: Record<string, string[]> = {
    period:       ["period","month","månad","date","datum","year","år","month_name","monthname","yearmonth","fiscal","bokföringsmånad","bokföringsdatum","transdate","trans_date","accounting_period","per"],
    account:      ["account","konto","kontonr","account_id","gl","glcode","gl_code","account_no","accountno","acc","acct","kontonummer","account_number","cost_account","huvudkonto","account_code"],
    account_name: ["account_name","kontonamn","name","description","descr","desc","benämning","text","label","account_desc","account_description","kontotext","gl_name","rubrik","category","kategori"],
    actual:       ["actual","utfall","fact","verklig","bokfört","belopp","amount","value","värde","actuals","outcome","utfall_sek","actual_amount","saldo","balance","summa","sum","total","kr","sek","netto"],
    budget:       ["budget","plan","planned","bud","target","mål","budget_amount","bud_amount","planerat","budgeterat","forecast","fc"],
    entity:       ["entity","bolag","company","legal","enhet","org","organisation","organization","company_name","legal_entity","subsidiary","dotterbolag","affärsområde"],
    cost_center:  ["cost_center","costcenter","cc","kostnadsställe","resultatenhet","department","dept","avdelning","ks","division","profit_center","profitcenter","enhet"],
    project:      ["project","projekt","proj","project_id","project_code","projectcode","project_name","projektnamn","proj_id"],
  };
  ...
  for (const { orig, lower } of colsLower) { if (lower === fieldClean) return orig; }
  for (const syn of synonyms) {
    const synClean = syn.replace(/[\s_\-\.]/g, "");
    for (const { orig, lower } of colsLower) {
      if (lower === synClean || lower.includes(synClean) || synClean.includes(lower)) return orig;
    }
  }
  if (preview.length > 0) {
...
function looksLikePeriod(v: string): boolean {
  return /^\d{4}-\d{2}$/.test(v) || /^\d{4}-\d{2}-\d{2}/.test(v) ||
    ...
    /^Q[1-4]\s?\d{4}/.test(v) || /^\d{6}$/.test(v);
}
function looksLikeAccount(v: string): boolean { return /^\d{4,6}$/.test(v.trim()); }
