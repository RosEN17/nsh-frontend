...
<!-- HEADER -->
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;padding-bottom:16px;border-bottom:3px solid #f59e0b">
  <div>
    ${logoHTML}
  ...
  <div style="text-align:right">
    <div style="font-size:22px;font-weight:800;color:#f59e0b;letter-spacing:1px">OFFERT</div>
...
<!-- OBSERVERA -->
${(result.warnings || []).length > 0 ? `<div style="padding:10px 16px;background:#fffbeb;border-left:3px solid #f59e0b;font-size:11px;color:#92400e;margin-bottom:12px"><strong>Observera:</strong><ul style="margin:4px 0 0 16px;padding:0">${result.warnings.map((w: string) => `<li>${w}</li>`).join("")}</ul></div>` : ""}
...
<!-- BETALNINGSUPPGIFTER -->
${paymentInfo ? `
<div style="margin-top:12px;font-size:10px;color:#aaa;text-align:center">
  ${paymentInfo}
</div>` : ""}
...
function EstimateInner() {
  ...
  async function handleGenerate() {
    ...
    try {
      const data = await createEstimate({ description: description.trim(), job_type: jobType || undefined, area_sqm: areaSqm ? parseFloat(areaSqm) : undefined, location: location || undefined, hourly_rate: parseFloat(hourlyRate) || 650, margin_pct: parseFloat(marginPct) || 15, include_rot: includeRot });
  ...
  function handleSave() {
    if (!result) return;
    saveEstimate({ id: crypto.randomUUID(), created: new Date().toISOString(), description: result.job_title || description, job_type: jobType, total_inc_vat: result.totals?.total_inc_vat || 0, customer_pays: result.totals?.customer_pays || result.totals?.total_inc_vat || 0, data: result });
    setSaved(true);
  }
    ...
    return (
      <ProtectedLayout>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <textarea className="input textarea" placeholder="T.ex: Badrumsrenovering 8 kvm. Riva befintligt kakel golv och väggar. Nytt tätskikt, klinker på golv (60x60), kakel på väggar (30x60). Ny dusch med glasvägg, ny toalett och handfat." value={description} onChange={(e) => setDescription(e.target.value)} rows={5} />
          {error && <div className="login-error" style={{ marginTop: 10 }}>{error}</div>}
        </div>
        <div className="grid-2" style={{ marginBottom: 16 }}>
          <div className="card card-sm"><label className="label">Yta (kvm)</label><input className="input" type="number" placeholder="T.ex. 8" value={areaSqm} onChange={(e) => setAreaSqm(e.target.value)} /></div>
          <div className="card card-sm"><label className="label">Plats</label><input className="input" placeholder="Stad eller postnr" value={location} onChange={(e) => setLocation(e.target.value)} /></div>
        </div>
        <div className="grid-3" style={{ marginBottom: 24 }}>
          <div className="card card-sm"><label className="label">Timpris (kr/h)</label><input className="input" type="number" value={hourlyRate} onChange={(e) => setHourlyRate(e.target.value)} /></div>
          <div className="card card-sm"><label className="label">Påslag (%)</label><input className="input" type="number" value={marginPct} onChange={(e) => setMarginPct(e.target.value)} /></div>
          <div className="card card-sm" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div><div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>ROT-avdrag</div><div style={{ fontSize: 11, color: "var(--text-faint)" }}>30% på arbete</div></div>
          </div>
        </div>
    ...
    return (
      <ProtectedLayout>
        <div className="loading-overlay">
          <svg width="48" height="48" viewBox="0 0 100 100" fill="none"><rect x="8" y="40" width="20" height="52" rx="3" fill="#f59e0b" opacity="0.6"/><rect x="34" y="24" width="20" height="68" rx="3" fill="#fbbf24"/><rect x="60" y="8" width="20" height="84" rx="3" fill="#f59e0b" opacity="0.6"/></svg>
  ...
  return (
    <ProtectedLayout>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div><div className="page-title">{result.job_title || "Kalkyl"}</div><div className="page-subtitle">{result.job_summary || description}</div></div>
        <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          ...
          <button className={`btn btn-sm ${saved ? "btn-secondary" : "btn-primary"}`} onClick={handleSave} disabled={saved}>{saved ? "✓ Sparad" : "Spara"}</button>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 16, padding: 0, overflow: "hidden" }}>
        <table className="est-table">
          <thead><tr>
            <th style={{ width: showSources ? "35%" : "40%" }}>Post</th><th>Enhet</th><th className="right">Antal</th><th className="right">À-pris</th><th className="right">Summa</th>
            {showSources && <th style={{ width: "20%" }}>Källa</th>}
          </tr></thead>
          <tbody>
            {(result.categories || []).map((cat: any, ci: number) => (
              <>
                <tr key={`cat-${ci}`} className="est-cat-row"><td colSpan={showSources ? 6 : 5}>{cat.name}</td></tr>
                {(cat.rows || []).map((row: any, ri: number) => (
                  <tr key={`row-${ci}-${ri}`}>
                    <td><div style={{ color: "var(--text-primary)", fontWeight: 500 }}>{row.description}</div>{row.note && <div style={{ fontSize: 11, color: "var(--text-faint)", marginTop: 2 }}>{row.note}</div>}</td>
                ...
                <tr className="est-subtotal"><td colSpan={showSources ? 5 : 4} style={{ textAlign: "right" }}>Delsumma {cat.name}</td><td className="right" style={{ fontFamily: "var(--mono)" }}>{fmtKr(cat.subtotal)}</td></tr>
              </>
            ))}
          </tbody>
        </table>
      ...
      {result.warnings?.length > 0 && <div className="warning-box" style={{ marginTop: 12 }}><strong>Observera:</strong><ul style={{ margin: "6px 0 0 16px", padding: 0 }}>{result.warnings.map((w: string, i: number) => <li key={i} style={{ marginBottom: 4 }}>{w}</li>)}</ul></div>}
      {result.assumptions?.length > 0 && <div style={{ marginTop: 12, padding: "12px 16px", background: "var(--bg-elevated)", border: "0.5px solid var(--border)", borderRadius: "var(--radius)", fontSize: 12, color: "var(--text-muted)" }}><strong style={{ color: "var(--text-secondary)" }}>Antaganden:</strong><ul style={{ margin: "6px 0 0 16px", padding: 0 }}>{result.assumptions.map((a: string, i: number) => <li key={i} style={{ marginBottom: 4 }}>{a}</li>)}</ul></div>}
    </ProtectedLayout>
  );
}
